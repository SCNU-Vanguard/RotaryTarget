...
uint8_t T_ADDR_0[5]={0x59, 0x51, 0x52, 0x21, 0x19};
uint8_t R_ADDR_0[5]={0x59, 0x51, 0x52, 0x21, 0x19};

uint8_t T_ADDR_1[5]={0xA1, 0xA1, 0xA1, 0xA1, 0xA1};
uint8_t R_ADDR_1[5]={0xA1, 0xA1, 0xA1, 0xA1, 0xA1};
...
//	W_Buf(W_REGISTER+RX_ADDR_P0, R_ADDR_0, 5);//配置接收通道0
	W_Reg(W_REGISTER+CONFIG,0x0E);//配置基本工作模式的参数;PWR_UP,EN_CRC,16BIT_CRC,接收模式,开启所有中断
	W_Reg(W_REGISTER+EN_AA,0x03);//通道0和通道1开启自动应答
//	W_Reg(W_REGISTER+RF_CH,80);//配置通信频率2.4G+80M
	W_Reg(W_REGISTER+RX_PW_P0,32);//配置接收通道0接收的数据宽度32字节
	W_Reg(W_REGISTER+EN_RXADDR,0x03);//接收通道0和接收通道1使能
	W_Reg(W_REGISTER+SETUP_RETR,0x13);//设置自动重发间隔时间:500us;最大自动重发次数:3次
	W_Reg(W_REGISTER+RF_SETUP,0x0E);//设置TX发射参数,0db增益,2Mbps,低噪声增益开启
	W_Reg(FLUSH_RX,NOP);
...
uint8_t Status;
uint8_t Send(uint8_t* Buf,int i)
{
	W_CE(0);
	if(i==0)
	{
