...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	...
	{
      get_data[0] = (RX_data[0]<<8)+RX_data[1];//×óÒ¡¸Ë×óÓÒÖµ
		  get_data[1] = (RX_data[2]<<8)+RX_data[3];//×óÒ¡¸ËÉÏÏÂÖµ
		  get_data[2] = (RX_data[4]<<8)+RX_data[5];//ÓÒÒ¡¸Ë×óÓÒÖµ
		  get_data[3] = (RX_data[6]<<8)+RX_data[7];//ÓÒÒ¡¸ËÉÏÏÂÖµ
